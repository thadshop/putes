handle_ssh_agent() {
    ...
    #   Usage: handle_ssh_agent [-v|--verbose]
    #   Without -v: narration is suppressed; a one-line status is still printed (success to stdout,
    #   failure to stderr). With -v, full narration goes to stdout and success has no extra summary.
    ...
    while [[ ${#} -gt 0 ]]; do
        case ${1} in
            ...
            --)
                ...
                ;;
            -*)
                echo "handle_ssh_agent: unknown option: ${1}" >&2
                return 1
                ;;
            *)
                echo "handle_ssh_agent: unexpected argument: $1" >&2
                return 1
                ;;
        esac
    done
    ...
    _rc=${?}
    if [[ ${_rc} -eq 0 ]]; then
        [[ ${OUTPUT} == '/dev/stdout' ]] || printf '%s\n' 'handle_ssh_agent: ok'
    else
        printf '%s\n' 'handle_ssh_agent: failed' >&2
    fi
    return "${_rc}"
}
