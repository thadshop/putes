...
# Check only: ./linux/setup.sh --check workstation
# Requires: bash, cp, mkdir, chmod, cat, mv, cmp, mktemp; optional: git for tracked-file listing.
...
usage() {
    echo "usage: ${0##*/} [-n|--dry-run] [--check] [--untracked=ask|include|exclude] workstation|server" >&2
}
...
# Subtrees under $HOME that should contain only files deployed by putes (extras are orphans).
PUTES_ONLY_REL_DIRS=(
    'etc/bash/rc'
    'lib/bash'
    'opt/python'
...
# Deployed paths that may legitimately differ from repo (runtime data, not config).
SKIP_CONTENT_COMPARE_RELS=(
    'log/tevent.log'
)

declare -A MANIFEST_KIND=()   # rel -> copy | assembled_bashrc | bashrc
declare -A MANIFEST_SRC=()    # rel -> absolute repo path (copy kind only)
declare -a CHECK_MISSING=()
declare -a CHECK_MODIFIED=()
declare -a CHECK_ORPHAN=()
...
rel_src_label() {
    local src="${1}"
    if [[ -n "${reporoot}" ]]; then
        printf 'putes:%s' "${src#"${reporoot}/"}"
    else
        printf '%s' "${src}"
    fi
}

write_assembled_bash_rc_to() {
    local out="${1}"
    local name="${2}"
    local base_f="${base_home}/etc/bash/rc/${name}"
    local prof_f="${prof_home}/etc/bash/rc/${name}"
    local -a srcs=("${base_f}")
    local src rel_src
...
manifest_add_copy() {
    ...
    local src="${2}"
    [[ -z "${rel}" ]] && return 0
    skip_home_deploy_rel "${rel}" && return 0
    MANIFEST_KIND["${rel}"]=copy
    MANIFEST_SRC["${rel}"]="${src}"
}

# Populate MANIFEST_* from git (or find fallback) for one home tree layer.
gather_home_tree_manifest() {
    local label="${1}"
    local home_root="${2}"
    local git_prefix="${3}"
    local copy_n='0'
    local path rel include_untracked local_src home_slash
    ...
    if [[ -n "${reporoot}" ]] && git -C "${reporoot}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        while IFS= read -r -d '' path; do
            ...
            manifest_add_copy "${rel}" "${reporoot}/${path}"
            copy_n=$((copy_n + 1))
    ...
    if [[ "${copy_n}" -eq 0 ]]; then
        home_slash="${home_root}/"
        while IFS= read -r -d '' local_src; do
            rel="${local_src#"${home_slash}"}"
            manifest_add_copy "${rel}" "${local_src}"
        done < <(find "${home_root}" -type f -print0 2>/dev/null || true)
    fi
}

gather_deploy_manifest() {
    local base_f name rel

    MANIFEST_KIND=()
    MANIFEST_SRC=()
    gather_home_tree_manifest "base" "${base_home}" "linux/base/home"
    gather_home_tree_manifest "${profile}" "${prof_home}" "linux/${profile}/home"
    ...
    for base_f in "${base_files[@]}"; do
        [[ -f "${base_f}" ]] || continue
        name="${base_f##*/}"
        rel="etc/bash/rc/${name}"
        MANIFEST_KIND["${rel}"]=assembled_bashrc
        unset "MANIFEST_SRC[${rel}]"
    done

    if [[ -r /etc/skel/.bashrc ]]; then
        MANIFEST_KIND['.bashrc']=bashrc
        unset 'MANIFEST_SRC[.bashrc]'
    fi
}

skip_content_compare_rel() {
    local rel="${1}" skip
    for skip in "${SKIP_CONTENT_COMPARE_RELS[@]}"; do
        [[ "${rel}" == "${skip}" ]] && return 0
    done
    return 1
}

compare_manifest_to_home() {
    local rel kind tgt expected tmp
    local -a rels=()
    ...
    for rel in "${rels[@]}"; do
        ...
        if skip_content_compare_rel "${rel}"; then
            continue
        fi
        tmp="$(mktemp)"
        case "${kind}" in
            copy )
                cp "${MANIFEST_SRC[${rel}]}" "${tmp}"
                ;;
            assembled_bashrc )
                write_assembled_bash_rc_to "${tmp}" "${rel##*/}"
                ;;
            bashrc )
                write_bashrc_to "${tmp}"
                ;;
    ...
    local dir orphan_rel f
    for dir in "${PUTES_ONLY_REL_DIRS[@]}"; do
        [[ -d "${tgtdir}/${dir}" ]] || continue
        while IFS= read -r -d '' f; do
            orphan_rel="${f#"${tgtdir}/"}"
            [[ -n "${MANIFEST_KIND[${orphan_rel}]}" ]] && continue
            CHECK_ORPHAN+=("${orphan_rel}")
        done < <(find "${tgtdir}/${dir}" -type f -print0 2>/dev/null || true)
    done

    if [[ ${#CHECK_ORPHAN[@]} -gt 0 ]]; then
        IFS=$'\n' CHECK_ORPHAN=($(printf '%s\n' "${CHECK_ORPHAN[@]}" | LC_ALL=C sort -u))
        unset IFS
    fi
}

print_deploy_check_report() {
    local rel total=0

    echo "INFO: deploy check profile=\"${profile}\"; target \"${tgtdir}\""

    if [[ ${#CHECK_MISSING[@]} -eq 0 && ${#CHECK_MODIFIED[@]} -eq 0 && ${#CHECK_ORPHAN[@]} -eq 0 ]]; then
        echo "INFO: deployed home matches repo (no differences)"
        return 0
    fi

    if [[ ${#CHECK_MISSING[@]} -gt 0 ]]; then
        echo "MISSING (in repo, not in \$HOME):"
        for rel in "${CHECK_MISSING[@]}"; do
            echo "  ${tgtdir}/${rel}"
        done
        total=$((total + ${#CHECK_MISSING[@]}))
    fi
    if [[ ${#CHECK_MODIFIED[@]} -gt 0 ]]; then
        echo "MODIFIED (differs from repo):"
        for rel in "${CHECK_MODIFIED[@]}"; do
            echo "  ${tgtdir}/${rel}"
        done
        total=$((total + ${#CHECK_MODIFIED[@]}))
    fi
    if [[ ${#CHECK_ORPHAN[@]} -gt 0 ]]; then
        echo "ORPHAN (in putes-only dirs, not in repo):"
        for rel in "${CHECK_ORPHAN[@]}"; do
            echo "  ${tgtdir}/${rel}"
        done
        total=$((total + ${#CHECK_ORPHAN[@]}))
    fi
    echo "INFO: ${total} difference(s) found"
    return 1
}

deploy_manifest_rel() {
    local rel="${1}"
    local kind="${MANIFEST_KIND[${rel}]}"

    case "${kind}" in
        copy )
            copy_one_home_file "${rel}" "${MANIFEST_SRC[${rel}]}"
            ;;
        assembled_bashrc )
            local out="${tgtdir}/${rel}"
            ensure_home_rel_dir "$(dirname "${rel}")"
            if [[ -n "${DRY_RUN}" ]]; then
                echo "DRY RUN: write \"${out}\" (assembled bash rc)"
            else
                write_assembled_bash_rc_to "${out}" "${rel##*/}"
                chmod 644 "${out}"
            fi
            ;;
        bashrc )
            if [[ -r /etc/skel/.bashrc ]]; then
                if [[ -n "${DRY_RUN}" ]]; then
                    ...
                    write_bashrc_to "${tgtdir}/.bashrc.tmp"
            ...
            ;;
        * )
            echo "WARN: unknown manifest kind for \"${rel}\"" >&2
            ;;
    esac
}

prompt_deploy_check_fixes() {
    local reply rel n i choice

    if [[ ${#CHECK_MISSING[@]} -eq 0 && ${#CHECK_MODIFIED[@]} -eq 0 && ${#CHECK_ORPHAN[@]} -eq 0 ]]; then
        return 0
    fi
    if [[ -n "${DRY_RUN}" ]]; then
        echo "INFO: dry run — not offering to fix differences"
        return 0
    fi

    echo
    echo "Fix differences?"
    echo "  [a] Deploy all missing/modified from repo"
    echo "  [s] Select files to deploy"
    echo "  [r] Remove all orphans"
    echo "  [o] Select orphans to remove"
    ...
    case "${reply}" in
        a | A )
            for rel in "${CHECK_MISSING[@]}" "${CHECK_MODIFIED[@]}"; do
                deploy_manifest_rel "${rel}"
            done
            prune_obsolete_etc_bash_rc
            ;;
        s | S )
            local -a deploy_rels=()
            deploy_rels=("${CHECK_MISSING[@]}" "${CHECK_MODIFIED[@]}")
            n=${#deploy_rels[@]}
            for i in "${!deploy_rels[@]}"; do
                rel="${deploy_rels[${i}]}"
                printf 'Deploy %s? [%d/%d y/n]: ' "${rel}" "$((i + 1))" "${n}"
                read -r choice </dev/tty
                case "${choice}" in
                    y | Y | yes ) deploy_manifest_rel "${rel}" ;;
                esac
            done
            prune_obsolete_etc_bash_rc
            ;;
        r )
            for rel in "${CHECK_ORPHAN[@]}"; do
                echo "rm: \"${tgtdir}/${rel}\""
                run_cmd rm -f "${tgtdir}/${rel}"
            done
            ;;
        o | O )
            n=${#CHECK_ORPHAN[@]}
            for i in "${!CHECK_ORPHAN[@]}"; do
                rel="${CHECK_ORPHAN[${i}]}"
                printf 'Remove orphan %s? [%d/%d y/n]: ' "${rel}" "$((i + 1))" "${n}"
                read -r choice </dev/tty
                case "${choice}" in
                    y | Y | yes )
                        echo "rm: \"${tgtdir}/${rel}\""
        ...
        q | Q | '' )
            ...
            ;;
        * )
            echo "INFO: unrecognized choice; no changes made"
            ;;
    esac
}
...
run_deploy_check() {
    ...
    compare_manifest_to_home
    print_deploy_check_report || rc='1'
    prompt_deploy_check_fixes
    return "${rc}"
}
...
assemble_etc_bash_rc() {
    ...
    for base_f in "${base_files[@]}"; do
        ...
        if [[ -n "${DRY_RUN}" ]]; then
            echo "DRY RUN: write \"${out}\" from: ${base_f}${prof_f:+ ${prof_f}}"
        else
            write_assembled_bash_rc_to "${out}" "${name}"
            chmod 644 "${out}"
        fi
